...
test('the gatekeeper is shown what was painted in the last day, newest first, first sentence only, at most 8', () => {
  const docs = [
    ...
    doc(5, 'After the Toast', 'Champagne glasses in a warm glow on a kitchen counter. Plates stacked.', 'queued'),
  ];
  ...
  assert.match(line, /- Last Light On: A desk lamp over a wooden desk\.\n/);
  assert.doesNotMatch(line, /Old|Declined|Failed|Seed|Plates stacked/);
...
test('the desk passes the day\'s work to the gatekeeper, and the prompt says what to do with it', () => {
  ...
  assert.match(artist, /what was painted today; choose a different light source and anchor object/);
});
