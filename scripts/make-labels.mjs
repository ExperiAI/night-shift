...
for (const [name, l] of Object.entries(labels)) {
  ...
  const shadow = l.shadow ? `<defs><filter id="s" x="-20%" y="-20%" width="140%" height="160%"><feDropShadow dx="0" dy="1" stdDeviation="2.2" flood-color="#06100f" flood-opacity="0.85"/></filter></defs>` : '';
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}">${shadow}<text x="${w / 2}" y="${Math.round(h * 0.68)}" text-anchor="middle" font-family="Iowan Old Style, Georgia, serif" font-style="italic" font-size="${l.size}" fill="${l.color}"${l.shadow ? ' filter="url(#s)"' : ''}>${l.text}</text></svg>`;
