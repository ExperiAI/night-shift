...
export async function studioStatus() {
  ...
  return {
    ...
    reels: posted.filter(c => c.film && c.mediaId && /\/reel\//.test(c.instagram ?? "")).sort((a, b) => (b.painted ?? '').localeCompare(a.painted ?? '')).map(c => { const i = insights?.get(c.mediaId!); return { id: c.id, title: c.take.title, at: c.painted, instagram: c.instagram, ...(i ? { views: i.views, reach: i.reach, held: i.held, avgWatchS: i.avgWatchS, skipRate: i.skipRate, shares: i.shares, saves: i.saves, comments: i.comments, syncedAt: i.syncedAt } : { views: null }) }; }),
