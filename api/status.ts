...
export async function studioStatus() {
  ...
  return {
    ...
    lastPosted: last ? { id: last.id, title: last.take.title, at: last.painted, instagram: last.instagram, film: last.film ?? null, filmMs: last.filmMs ?? null, filmError: last.filmError ?? null, postedAs: last.postedAs ?? null, /* the reveal (docs/reveal.md): the film, how long the server took to make it, and the SHAPE Instagram took — a refused carousel falls back to the plain Reel and would otherwise do so unseen */ captionOnInstagram: captionMatches(last) == null ? 'not read back yet' : captionMatches(last) ? 'matches what was sent' : 'DIFFERS from what was sent' } : null, // issue #22: a publish that cannot be read back is a claim
    ...
    // every posted film with what Instagram reports for it (#11): views, reach, and how much of the film was
    // watched. The number the score is judged by; read it here instead of the app's insights by hand. Newest first.
    // The test is "did the POST carry the film", which is not the same as "does this painting have one" —
    // a still post with a film backfilled later is not a filmed post. That used to be read off a /reel/
