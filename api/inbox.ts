...
// commissions through the same desk as the API and MCP.
...
import { receive } from './_lib/desk.js';
import { EMPTY_STATE, freshItems, remember, replyFor, reactionSystemPrompt, type InboxItem, type InboxState, type Reaction } from './_lib/react.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  for (const it of fresh) {
    ...
    const r = await chatJSON<Reaction>(reactionSystemPrompt(), `${it.kind === 'dm' ? 'Direct message' : 'Comment'} from @${it.handle}: ${it.text.slice(0, 600)}`, REACT_MODEL);
    let text = '';
    ...
    if (r.kind === 'commission' && r.commission) {
      if (igCommissionsToday >= MAX_INSTAGRAM_COMMISSIONS_PER_DAY) text = replyFor(r, null, "The studio is full for today. Ask me again tomorrow.");
        ...
        try {
          const receipt = await receive(r.commission, `@${it.handle}`, origin);
